/*
modulo angular para traer datos de usuario del frontend
*/

'use strict';

angular.module('myApp.user2', ['ngRoute'])

.config(['$routeProvider', function($routeProvider) {
  $routeProvider.when('/user2/:userLogin', {
    templateUrl: 'user/index.html',
    controller: 'ctrlUser2'
  });
  /*TODO. se puede usar esta redireccion para borrar usuarios no deseados, que lleguen por spam o robots 
  $routeProvider.when('/user2/delete/:userLogin', {
    templateUrl: 'user/user.html',
    controller: 'ctrlUser2Delete'
  });
  */
}])
...
/*controlador de modulo user2*/
.controller('ctrlUser2', ['$scope', '$http', '$routeParams', function($scope, $http, $routeParams) {
  var ctrl = this;
  $scope.user = [];
  ...
  $scope.showData = function() {
   $http.get("./user/get_user_frontend.php?login=" + $scope.login)
