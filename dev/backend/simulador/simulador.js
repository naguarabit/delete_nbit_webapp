...
.config(['$routeProvider', function($routeProvider) {
  $routeProvider.when('/simulador', {
    ...
    controller: 'CtrlSimulador'
  });
}])



.controller('CtrlSimulador', ['$scope', '$http', '$timeout', '$q', function($scope, $http, $timeout, $q) {

//trae info de la API Yadio, con tasas promedio
//guarda en la variable data
//luego de traer la data, calcula los montos de remesa con valor inicial de montoUSD
...
//calcular montoBTC (monto en bitcoins) basado en monto USD y tasa en dolares
...
//TODO.
//calcula monto en bitcoins, basado en monto en origen
//esto es: calcula montobtc basado en monto2
//montobtc = monto2 * (valor de 1USD en Gs) * (valor de 1$ en Bs "a la venta")
$scope.calcularMontoBTC_1 = function() {
 console.log('simulador. calcularMontoBTC_1. inicio');
 console.log('monto1=' + $scope.monto1);
 console.log('TASA BTC a PYG, sin comision=' + $scope.BTC_USD_price );
  //validacion
  if (!$scope.monto1 || $scope.monto1 == 0 || $scope.monto1 == ""){
    $scope.montobtc = 0;
    return false;
  }

 var montobtc = 0;
 $scope.tasa_BTC_PYG = $scope.BTC_USD_price * $scope.USD_PYG; 
 var montobtc = $scope.monto1 / $scope.tasa_BTC_PYG;
 $scope.montobtc = parseFloat(montobtc).toFixed(8); //redondeo a 8 decimales
 console.log('simulador. calcularMontoBTC_1. tasa_BTC_PYG=' + $scope.tasa_BTC_PYG);
 console.log('simulador. calcularMontoBTC_1. montobtc=' + $scope.montobtc);
...
$scope.aplicarComision = function(){
  ...
  var montoBTC_final  = $scope.montobtc * (1 - $scope.porc_comision/100);
  ...
  $scope.montoBTC_final = montoBTC_final.toFixed(8); //round a 8 decimales
