...
$sql = "SELECT f.id, f.codigo, f.nombre, f.codpais, f.cod_banco, f.doctitular, f.nombretitular, \n".
"f.nrocuenta, f.tipocuenta, f.tipocuenta_desc, f.ciudad, f.observ, f.activo, p.nombre as nombre_pais, \n".
"bp.nombre as banco_nombre, \n".
"f.activo as estatus_activo \n".
"FROM formapago_origen f \n".
"LEFT JOIN banks_pais bp ON bp.codigo = f.cod_banco \n".
"LEFT JOIN pais p ON p.codigo = f.codpais \n".
//"WHERE p.activo AND f.activo \n".
"WHERE p.activo \n".
"ORDER BY f.nombre";
...
while($rs = $result->fetch_array(MYSQLI_ASSOC)) {
  ...
  $outp .= ',"banco_nombre":"'    . $rs["banco_nombre"]   .'"';
